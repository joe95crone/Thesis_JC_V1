...
\end{equation}
This results in myriad combinations of $\beta^{*}$ and $\theta_{\mathrm{col}}$ that satisfy a particular chosen bandwidth larger than the lower limit (Eq.~\ref{eq:bandwidth_limitation_minimum}).
...
The genetic algorithm (GA) optimisation uses the minimisation routine of the strength Pareto evolutionary algorithm \cite{} within the \textsc{PISA} platform \cite{} framework. The objective functions minimised are the \texit{rms} Bandwidth (Eq.~\ref{eq:rms_bandwidth}) and $-\mathcal{F}_{\mathrm{col}}$ (Eq.~\ref{eq:collimated_flux}), the negative collimated flux (hence it is maximised) for the tuning curve case. Whereas the single point optimisation is conducted using the penalty function (Eq.~\ref{eq:penalty_function}) and the negative collimated flux as objective functions. In the case of the single point bandwidth optimisations the minimimum penalty function point is selected as the optimum solution where the associated variables of this point are the optimal configuration of the ICS source.     

The genetic algorithm method is a global optimisation technique which provides a set of solutions for each optimisation run, allowing the full solution space to be mapped more effectively. Global optimisation via a genetic algorithm should deliver a global minimum for the bandwidth whilst providing a global maximum for the collimated flux. This method should avoid the pitfalls of local optimisation techniques where a local minima may be returned.
