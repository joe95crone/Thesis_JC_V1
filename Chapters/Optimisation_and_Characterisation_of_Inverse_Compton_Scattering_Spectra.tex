...
\end{equation}
where all symbols are consistent with the exception of $\sigma_{x}$, which in the work of Curatolo et al is the \textit{rms} spot size of the electron bunch at the IP and $\Psi = \gamma\theta_{\mathrm{col}}$, the acceptance angle.
...
\caption{aye \textcolor{blue}{**ADD THE ICARUS RUNS TO THESE PLOTS, LIKE BALSA HAS WITH THE BW COMPARISON**}}
\label{fig:curatolo_collimated_flux_comparison}
\end{figure}

\textcolor{blue}{**Show Curatolo collimated Flux** \\ **Say that had to do head-on and exclude case B as their luminosity assumes round beams** \\ **They don't show a derivation** \\ **Show plots of A and C** \\ **Add on the ICARUS data for these** \\ **Explain that Ranjan et al Showed a similar situation for bandwidth + link to that theory section**}

\textcolor{blue}{Mention that the Curatolo formula is deficient as it doesn't provide adequate derivation to verify, seemingly that the bandwidth it is based upon is deficient (Ranjan et al say so in (2018), Fig.~1) and doesn't provide angular case either. Do a comparative plot of a test case for my analytical formula and curatolo's analytical formula.}
