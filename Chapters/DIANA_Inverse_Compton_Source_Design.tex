...
Whilst the ERL approach has been shown to be competitive in terms of flux the real advantage of an ERL driven ICS source may be producing a high flux in a narrow bandwidth, and even achieving a narrowband . However, quantifying the bandwidth and the collimated flux per bandwidth is difficult as these are often not quoted within the literature.

\textcolor{blue}{why is bandwidth important to a gamma source? \\ storage rings have high energy spread of electron bunch \\ FEL as photon sources give bad bandwidth \\ can be overcome - NEQ ring + low emittance rings \\ not enough info to be decisive but ERL are promising}       
