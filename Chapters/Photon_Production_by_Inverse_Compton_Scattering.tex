...
Radiation is emitted when a charged particle is accelerated. Imagine a charged particle is being subjected to a small, uniform acceleration $a$ for a time $\Delta t$ after which it travels at constant velocity $u=a\Delta t$ where $u\ll c$. A shown in Fig.~\textcolor{blue}{**LINK**}, in the rest frame of the charged particle a radial electric field is produced however, in the laboratory frame the electric field is no longer completely radial and there is a kink corresponding to the acceleration of the charge. Note that the field lines must remain continuous because of Gauss' law in free space $\Delta\cdot\boldsymbol{E}=0$. The kink in the electric field corresponds to the emitted radiation.
\textcolor{blue}{**REST FRAME E FIELD AND LAB FRAME ACCELERATED E FIELD DIAGRAM**}
Rather than calculate the radiation properties for this scenario we instead consider an alternative scenario more appropriate to modelling radiation incident on charged particles where the acceleration applied to the charged particle causes the charged particle to oscillate. This is analogous to the well-known Hertzian dipole. A hertzian dipole emits radiation at the identical frequency to the frequency of the driving radiation and the polarisation of the incident radiation is also conserved by the emitted radiation. Within the rest of this Chapter, electrons are the only charged particle considered. 

The simplest form of radiation scattering interaction is Thomson scattering \cite{thomson1904xxxiv} between radiation incident upon an electron. The electron is treated classically because the incident radiation is much lower energy $E_{L}=hf$, with $h$ Planck's constant and $f$ the frequency, than the rest mass of the electron $m_{e}c^{2}$ ($m_{e}c^{2}\gg h\nu$). The incident radiation causes the electron to oscillate and therefore be accelerated, much like the previous Hertzian dipole case. The accelerated electron subsequently emits radiation, which is emitted isotropically in the electron frame. The radiation is emitted at an identical frequency to the incident radiation and therefore no energy is transferred; the scattering is elastic. Polarisation of the incident radiation is conserved as in the Hertzian dipole case.     
