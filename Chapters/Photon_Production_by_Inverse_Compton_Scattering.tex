...
\end{equation}
where $\mathcal{F}_{0.1\%}$ is the flux in a 0.1\% FWHM bandwidth (Eq.~\ref{eq:flux_0.1_bandwidth}), $\sigma_{\gamma,x/y}$ are the source sizes in each plane (Eq.~\ref{eq:source_size}) and $\sigma_{\gamma,x/y}'$ are the source angular divergences (Eq.~\ref{eq:source_divergence}) in each plane. Assuming a non-diffraction limited electron bunch and that, as is typical $\sigma_{e,x/y}' > \sigma_{L}'$ the brilliance can be roughly calculated as
\begin{equation}
...
\textcolor{blue}{**DIFFERENT FORMS OF BRILLIANCE FROM KIRSTEN** \\ **EVALUATE EACH, SEE COMPTONCALCS2**}

\textcolor{blue}{**FV HARTEMANN PEAK BRILLIANCE** \\ **LIMITATIONS?**}
